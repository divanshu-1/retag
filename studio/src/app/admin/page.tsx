...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    apiRequest('/sell/admin/pending')
      ...
      .then(data => {
        console.log('Admin pending data:', data);
        setRequests(Array.isArray(data?.products) ? data.products : []);
        setLoading(false);
      })
      .catch((error) => {
  ...
  const fetchListed = async () => {
    ...
    try {
      ...
      console.log('Admin listed data:', data);
      setListed(Array.isArray(data?.products) ? data.products : []);
